				...
				<block v-for="(item, index) in cartList" :key="item.id">
					...
					>
						...
						<view class="item-right">
							<text class="clamp title" style="display: block;">{{item.productName}}</text>
							<text class="attr">{{item.attr_val}}</text>
							<text class="price">¥{{item.price}}</text>
							<!-- TODO 暂定最大库存为99，应该动态获取真实库存信息 -->
							<uni-number-box 
								class="step"
								:min="1" 
								:max="99"
								:value="item.quantity>99?99:item.quantity"
								:isMax="item.quantity>=99?true:false"
...
<script>
	...
	export default {
		...
		computed:{
			// ...mapState(['hasLogin'])
			...mapGetters(['hasLogin']),
			...mapState(['cartInfo','orderCartInfo'])
		},
		methods: {
			//请求数据
			async loadData(){
				let list = this.cartInfo
				if(!list || !list.length) {
					this.empty=true
					return
				}
				let cartList = list.map(item=>{
					item.attr_val = reduce.convert({ 'cap': false })((res,obj,index)=>{ 
						res += ' '+obj.value+' ';
						return res
			...
			check(type, index){
				...
				this.$store.commit("SET_CARTINFO",this.cartList)
				this.calcTotal(type);
			},
			//数量
			numberChange(data){
			...
			deleteCartItem(index){
				...
				if(id) {
					...
					this.$api.delCartInfo(param,res=>{
						if(res.code == 200) {
							this.$api.msg('删除成功')
						}else {
							this.$api.msg(res.message)
			...
			clearCart(){
				uni.showModal({
					...
					success: (e)=>{
						if(e.confirm){
							this.$api.clearCartInfo(res=>{
								if(res.code == 200) {
									this.$api.msg('清空购物车成功')
									this.cartList = [];
									this.$store.commit("SET_CARTINFO",[])
								}else {
									this.$api.msg(res.message)
								}
							})
			...
			//创建订单
			createOrder(){
				let toOrderCartInfolist = this.cartList.filter((element)=>element.checked);
