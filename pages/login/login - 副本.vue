	...
	<view class="content login-page">
		<view class="input-group">
			<view class="input-row border-bottom-1px">
				...
				<input type="number" clearable v-model="params.phoneNo" placeholder="请输入手机号" :maxLength="11" :focus="true"></input>
			</view>
			<view class="input-row border-bottom-1px">
				<text class="title">验证码：</text>
				<input class="uni-input" type="number" v-model="params.code" placeholder="请输入短信验证码" :maxLength="6"/>
	...
	export default {
		data() {
			return {
				...
				reqBody: {},
				// 如果为登录状态 默认跳转的地址
				redirectRoute: this.$mRoutesConfig.user,
				routeQuery: {}
			}
		},
		onLoad(query) {
		...
		methods: {
			...
			async bindLogin() {
				try {
					...
					let userInfo = await this.$api.postLogin(this.reqBody);
