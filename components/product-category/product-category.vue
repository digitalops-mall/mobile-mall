<template>
	<view class="content" >
		<scroll-view scroll-y class="left-aside" >
			...
			<text @click="close">返回</text>
			<view v-for="item in flist" :key="item.id" class="f-item b-b" :class="{active: item.id === currentId}" @click="tabtap(item)">
				{{item.name}}
			</view>
		</scroll-view>
		<scroll-view scroll-with-animation scroll-y class="right-aside" @scroll="asideScroll" :scroll-top="tabScrollTop" @touchmove.stop="">
	...
	export default {
		...
		methods: {
			async loadData(){
				...
				let that = this;
				let list = null;
				let url = this.$mConfig.baseUrl+"/home/productAllCateList"
				http.GET(url, list, {}, (res) => {
				    console.log(res)
